...
{{-- Responsive popup shell: a card that hangs with margin on every side (even on mobile),
     centered on desktop. Expects an ancestor with x-data exposing `open`. --}}
<div x-show="open" x-cloak class="fixed inset-0 z-[100] flex flex-col items-center justify-end p-3 sm:justify-center sm:p-4" x-transition.opacity>
    ...
    <div x-show="open"
         ...
         {{ $attributes->merge(['class' => "glass-strong relative w-full max-h-[85vh] overflow-y-auto rounded-3xl p-6 shadow-2xl sm:max-h-none sm:overflow-visible $maxWidth"]) }}>
        {{ $slot }}
    </div>
</div>
