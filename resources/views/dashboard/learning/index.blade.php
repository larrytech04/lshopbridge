@extends('layouts.app')
@section('page-title', __('Learning center'))
...
<div class="space-y-8">
    ...
    <div class="relative overflow-hidden rounded-3xl border border-white/10 bg-brand-900 p-8 text-white sm:p-10">
        ...
        <div class="relative z-10 max-w-2xl">
            <span class="pill bg-white/15 text-white ring-1 ring-white/20">{{ __('Free for every member') }}</span>
    ...
    <div class="no-scrollbar flex items-center gap-2 overflow-x-auto pb-1">
        <a href="{{ route('learning.index') }}" class="shrink-0 rounded-full border px-3.5 py-1.5 text-sm font-semibold transition {{ !$category ? 'border-slate-900 bg-slate-900 text-white' : 'border-app text-body hover:surface-2' }}">{{ __('All') }}</a>
        @foreach ($allCategories as $c)
            @php $meta = guide_category_meta($c); @endphp
            <a href="{{ route('learning.index', ['category' => $c]) }}" class="flex shrink-0 items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-sm font-semibold transition {{ $category === $c ? 'border-slate-900 bg-slate-900 text-white' : 'border-app text-body hover:surface-2' }}">
        ...
        <div>
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($guides as $guide)
                    <a href="{{ route('learning.show', $guide) }}" class="group flex flex-col rounded-3xl border border-app p-5 transition hover:-translate-y-0.5 hover:shadow-lg">
                        ...
                        <h3 class="mt-4 font-semibold text-strong group-hover:text-brand-500">{{ $guide->title }}</h3>
                        <p class="mt-1.5 line-clamp-2 flex-1 text-sm text-muted">{{ $guide->excerpt }}</p>
