    ...
    <div x-show="tab === 'devices'" x-cloak class="mt-4 space-y-4">
        <div class="rounded-3xl border border-app p-6">
            <div class="flex items-center gap-3">
                <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-slate-500/12 text-slate-500"><x-icon name="shield" class="h-5 w-5" /></span>
                ...
                <span class="pill shrink-0 bg-slate-400/15 text-[10px] font-bold uppercase text-slate-500 ring-1 ring-slate-400/30">{{ __('Coming soon') }}</span>
            </div>
        </div>
        ...
        <div class="rounded-3xl border border-app p-6">
            <p class="font-semibold text-strong">{{ __('This device') }}</p>
            <p class="mt-2 flex items-center gap-1.5 text-sm text-muted">
                <x-icon name="clock" class="h-4 w-4 shrink-0" />
                {{ $user->last_login_at ? __('Last active :time', ['time' => $user->last_login_at->diffForHumans()]) : __('No login activity recorded yet') }}
                @if ($user->last_login_ip)<span class="text-faint">· {{ $user->last_login_ip }}</span>@endif
            </p>
        </div>
    </div>
</div>
